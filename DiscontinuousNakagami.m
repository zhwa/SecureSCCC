...
%    < Naka = DiscontinuousNakagami(1.35,1,50,100); >
% 2) To see also the distributions of the channel described, type, e.g.:
%    < [Naka,f1,f2] = DiscontinuousNakagami(1.35,1,500,10000); >
...
function Npower = NakagamiEnvelopeDistribution(m,Omega,rVec)

if m ~= 0.5,
    Npower = (2/gamma(m))*((m/Omega)^m)*rVec.^(2*m-1).*exp(-(m/Omega)*rVec.^2);
else
    Npower = sqrt(2/(pi*Omega))*exp(-(m/Omega)*rVec.^2);
end

% --------------------------------------------------------------------------------

function [H,X,L,U] = Histogram(Data,Nbins,L,U)
...
n = 0:Nbins;
X = L + (n + 0.5)*Delta;
H = (Nbins/length(Data))*hist(Data,X);

% --------------------------------------------------------------------------------
%
% Below are functions used to construct the phase distribution plots
%
% --------------------------------------------------------------------------------

function Nphase = NakagamiPhaseDistribution(m,thetaVec)

if nargin == 1,
    thetaVec = linspace(0,pi,100);
else
    Nphase = gamma(m)*(abs(sin(2*thetaVec)).^(m-1))/((2^m)*gamma(m/2)^2);
end
